...
python utils_video.py -data=data/scanner3D/stand/train -mode=output-scanner -folder=data/scanner3D/renaissance_03/train -radius=30 -frames=30


python utils_video.py -data=data/replica/room_0/train -mode=output-replica -folder=data/replica/room_0/train -radius=30 -frames=30

    """
    if args.mode == 'output-scanner':
        dataset = ScannerDataset(args.data, read_points=True, sample_rate=0.1)
        # vis for debug
        points = dataset.dense_points
        points = vedo.Points(points, r=2)
        # a = dataset.cameras
        cameras = get_vedo_cameras(dataset.R, dataset.T, arrow_len=0.5)  # get_vedo_cameras(cam_R, cam_T, arrow_len=0.5)
        vedo.show(points, cameras, axes=1)
